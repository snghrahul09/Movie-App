...
export default class Movies extends Component {
    ...
    render() {
        console.log('render');
        let { movies, currSearchText, currPage, genres, cGenres } = this.state; //ES6 destructuring
        let limit = 4;
        let filteredArr = [];
        if (currSearchText === '') {
            filteredArr = movies;
                        ...
                        <div className='row'>
                            <div className='col-3'>
                                <ul className="list-group">
                                    {
                                        genres.map((genreObj) => (
                                            <li onClick={() => this.handleGenreChange(genreObj.name)}
                                             key={genreObj._id} className='list-group-item'>
                                                {genreObj.name}</li>
                                        ))
                                    }
                                </ul>
                                <h5>Current Genre: {cGenres}</h5>
                            </div>
                            <div className='col-9'>
                            ...
                            <span
                                ...
                                <input type='search' value={this.state.currSearchText} onChange={this.handleChange} >
                                </input>
                                </div>
                                    ...
                                    <table className="table">
                                        <thead>
                                            <tr>
                                                ...
                                                <th scope="col">
                                                    <i onClick={this.sortByStock} className="fas fa-caret-up" aria-hidden="true"></i>
                                                    Stock
                                                    <i onClick={this.sortByStock} className="fas fa-caret-down" aria-hidden="true"></i>
                                                </th>
                                                <th scope="col">
                                                    <i onClick={this.sortByRatings} className="fas fa-caret-up" aria-hidden="true"></i>
                                                    Rate
                                                    <i onClick={this.sortByRatings} className="fas fa-caret-down" aria-hidden="true"></i>
                                                </th>
                                                <th></th>
                                            </tr>
                                        </thead>
                                    ...
                                    <ul className="pagination">
                                        {
                                            pageNumberArr.map((pageNumber) => {
                                                ...
                                                return (
                                                    <li key={pageNumber} onClick={() => this.handlePageChange(pageNumber)} className={classStyle}>
